...
For each of the three pairs, Bhoomi Setu calculates `|A - B| / max(A, B) × 100`, making the percentage deterministic and independent of source order. The explicit `BHOOMI_SETU_DEMO_AREA_V1` policy classifies ≤2% as `CONSISTENT`, >2% through ≤5% as `REVIEW`, and >5% as `POTENTIAL_ISSUE`; unavailable inputs are `INSUFFICIENT_EVIDENCE`. These are demo tolerances only, not legal, cadastral, statutory, or government tolerances. These derived comparison results remain separate from Phase 5 `VerificationResult` data and are never ownership conclusions.
